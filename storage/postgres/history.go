...
func (r *historyRepo) HistoryMessage(ctx context.Context, req *coins_service.HistoryUserRequest) (*coins_service.HistoryMessageResponse, error) {
	var (
		query = `
			SELECT
				ut."id",
	...
	for rows.Next() {
		var (
			id, name, coin_id, user_id, user_confirmation_img,
			coin_price, coin_amount, all_price, status, user_address,
			card_name, payment_card, message, transaction_status, created_at sql.NullString
		)

		if err := rows.Scan(
			&id, &name, &coin_id, &user_id, &user_confirmation_img,
			&coin_price, &coin_amount, &all_price, &status, &user_address,
			&card_name, &payment_card, &message, &transaction_status, &created_at,
		); err != nil {
			return nil, err
		}
		...
		var queryMessage = `
			...
			WHERE pm.user_id = $1 AND pm.user_transaction_id = $2
		`
		var text, read, file sql.NullString
		err = r.db.QueryRow(ctx, queryMessage, req.UserId, id.String).Scan(&text, &read, &file)
		if err == sql.ErrNoRows {
			result.HistoryStatus = nil
		} else if err != nil {
			return nil, err
		} else {
			result.HistoryStatus = &coins_service.TransactionStatus{
				Text:    text.String,
				Status:  read.String,
				Message: file.String,
			}
	...
	return &coins_service.HistoryMessageResponse{
		HistoryWithStatus: data,
	}, nil
}
